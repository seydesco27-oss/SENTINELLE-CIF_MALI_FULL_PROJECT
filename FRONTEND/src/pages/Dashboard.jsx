...
/** Options alignées SEED_01 (caisses/agences) + aml_rules réelles — pas de labels inventés */
const FILTER_OPTIONS = {
  ...
  caisse: [
    ...
    ["CIF Bamako Centre", "CIF-SEED-BAM"],
    ["CIF Ségou", "CIF-SEED-SGO"],
  ],
  agence: [
    ["Toutes les agences", "ALL"],
    ["Bamako Hippodrome", "AG-SEED-BAM-01"],
    ["Bamako ACI 2000", "AG-SEED-BAM-02"],
    ["Ségou Centre", "AG-SEED-SGO-01"],
  ],
  risk: [
    ["Tous risques", "ALL"],
    ["Critique", "CRITICAL"],
  ...
  type: [
    ["Tous types", "ALL"],
    ["Montant unitaire élevé", "LARGE_AMOUNT"],
    ["Fractionnement", "STRUCTURING"],
    ["Transferts rapides", "RAPID_TRANSFER"],
    ["Compte dormant", "DORMANT_ACCOUNT"],
    ["Volume inhabituel", "UNUSUAL_VOLUME"],
    ["Forte activité espèces", "HIGH_CASH_ACTIVITY"],
    ["Corridor à risque", "HIGH_RISK_CORRIDOR"],
    ["Seuil CENTIF 15M", "CENTIF_DAILY_15M"],
  ],
  status: [
    ["Tous statuts", "ALL"],
    ["Ouvert", "OPEN"],
    ["En analyse", "IN_REVIEW"],
    ["Clos", "CLOSED"],
    ["Écarté", "DISMISSED"],
  ],
};
...
export default function Dashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const trendDays =
          filters.period === "today" ? 1 : filters.period === "30d" ? 30 : 7;
  ...
  const filteredAlerts = useMemo(() => {
    ...
    let minDate = null;
    if (filters.period === "today") minDate = startOfToday;
    else if (filters.period === "7d") minDate = new Date(startOfToday.getTime() - 6 * 86400000);
    else if (filters.period === "30d") minDate = new Date(startOfToday.getTime() - 29 * 86400000);

    return priorityAlerts.filter((alert) => {
      if (minDate && alert.created_at) {
        const d = new Date(alert.created_at);
        if (Number.isNaN(d.getTime()) || d < minDate) return false;
      }
            ...
            <div className="dashboard-heading-actions">
              ...
              <div className="system-live system-live-dual" title="Couches séparées : AML déterministe (BD) ≠ assistance ML">
                <div className="system-live-row">
                  ...
                  <span className="system-live-label">AML</span>
                  <small>{error ? "API indisponible" : "Règles & procédures actives"}</small>
                </div>
                <div className="system-live-row">
