...
def _random_montant(client_profil: str = None) -> float:
    # une petite fraction de transactions legitimes de montant eleve
    # (gros decaissements de credit, depots d'affaires) - pour eviter que
    # "proche du seuil" devienne un signal trop parfait de fraude
    if rng.random() < cfg.TAUX_TRANSACTIONS_LEGITIMES_ELEVEES:
        return float(rng.uniform(3_000_000, cfg.SEUIL_DECLARATION_FCFA * 0.97))
...
def inject_structuring(client_id: str, date_fin) -> list[dict]:
    ...
    for i in range(n_operations):
        montant = cfg.SEUIL_DECLARATION_FCFA * rng.uniform(cfg.STRUCTURING_MIN_RATIO, cfg.STRUCTURING_MAX_RATIO)
        tx.append({
            "transaction_id": str(uuid.uuid4())[:8],
            "client_id": client_id,
