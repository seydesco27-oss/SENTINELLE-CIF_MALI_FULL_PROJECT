...
# --- Correctif de compatibilite Python 3.11+ ---
# random.sample() n'accepte plus les sets/dict_keys directement depuis
# Python 3.11 (le code original de 2019-2021 s'appuyait sur l'ancien
# comportement). On convertit automatiquement en liste avant l'appel.
_original_random_sample = random.sample
def _compatible_sample(population, k):
    if not isinstance(population, (list, tuple)):
        population = list(population)
    return _original_random_sample(population, k)
random.sample = _compatible_sample
# --- fin correctif ---
import csv
import json
import os
