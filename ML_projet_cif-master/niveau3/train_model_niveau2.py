"""
train_model_niveau2.py - Entraînement sur données AMLSim (Niveau 2)

Identique à train_model.py mais utilise les fichiers Niveau 2 :
- transactions_n2.csv (transactions AMLSim enrichies)
- clients_n2.csv (comptes AMLSim)
"""
...
def load_dataset():
    """Charger les données Niveau 2 (AMLSim)"""
    clients = pd.read_csv(BASE_DIR / "clients_n2_adapted.csv")
    transactions = pd.read_csv(BASE_DIR / "transactions_n2.csv", parse_dates=["date_transaction"])
...
def train_model_with_kfold():
    ...
    print("=" * 78)
    print("ENTRAINEMENT NIVEAU 2 - DONNÉES AMLSIM (IBM Research)")
    print("=" * 78)
    ...
    print(f"  - Classe 1 (risqué)  : {(base['is_risky'] == True).sum()} ({(base['is_risky'] == True).mean() * 100:.2f}%)")
    ...
    print(f"\nClients partagés CV/holdout : {len(overlap)} (attendu : 0)")

    preprocessor = _make_preprocessor()

    print("\n" + "=" * 78)
    print("VALIDATION CROISÉE (K-FOLD PAR CLIENT, k=5)")
    print("=" * 78)
    ...
    print("\n" + "=" * 78)
    print("MODELE FINAL (ENTRAINÉ SUR TOUT LE CV SET)")
    print("=" * 78)
