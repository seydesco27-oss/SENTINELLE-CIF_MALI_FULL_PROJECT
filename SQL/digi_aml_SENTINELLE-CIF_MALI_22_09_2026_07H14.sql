...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_account_open` (IN `p_client_id` BIGINT, IN `p_account_number` VARCHAR(50), IN `p_account_type` VARCHAR(50), IN `p_opening_balance` DECIMAL(18,2), IN `p_currency` CHAR(3), IN `p_account_manager_id` BIGINT, OUT `p_account_id` BIGINT, OUT `p_status` VARCHAR(30), OUT `p_message` VARCHAR(500))   main_acc:
BEGIN
...
DROP PROCEDURE IF EXISTS `sp_aml_evaluate_full`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_aml_evaluate_full` (IN `p_transaction_id` BIGINT)   BEGIN
  CALL sp_aml_engine_execute(p_transaction_id);
  CALL sp_aml_rule_centif_daily_15m(p_transaction_id);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_aml_evaluate_transaction` (IN `p_transaction_id` BIGINT)   BEGIN
  CALL sp_aml_evaluate_full(p_transaction_id);
  ...
  SELECT
    ...
    t.amount,
    (
      SELECT GROUP_CONCAT(ar.rule_code ORDER BY ar.rule_code SEPARATOR ',')
      FROM rule_executions re
      JOIN aml_rules ar ON ar.id = re.rule_id
      WHERE re.transaction_id = t.id
        AND UPPER(COALESCE(re.execution_result, '')) = 'MATCH'
    ) AS matched_rules
  FROM transactions t
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_aml_evaluate_transaction_silent` (IN `p_transaction_id` BIGINT)   BEGIN
  CALL sp_aml_evaluate_full(p_transaction_id);
END$$
...
BEGIN
  ...
  DECLARE cur_transactions CURSOR FOR
    ...
    WHERE t.transaction_status = 'COMPLETED'
      AND (
        SELECT COUNT(*)
        FROM rule_executions re
        INNER JOIN aml_rules ar ON ar.id = re.rule_id
        WHERE re.transaction_id = t.id AND ar.active = 1
      ) < v_active_rules
    ORDER BY t.id
    LIMIT p_batch_size;
  ...
  IF p_batch_size IS NULL OR p_batch_size <= 0 THEN
    SELECT 'ERROR' AS status, 'La taille du batch doit être supérieure à 0.' AS message;
    LEAVE main_proc;
  END IF;

  SELECT COUNT(*) INTO v_active_rules FROM aml_rules WHERE active = 1;
  ...
  read_loop: LOOP
    FETCH cur_transactions INTO v_transaction_id;
    IF v_done = 1 THEN
      LEAVE read_loop;
    END IF;

    CALL sp_aml_evaluate_full(v_transaction_id);
  ...
  SELECT
    'SUCCESS' AS status,
    v_processed AS processed,
    v_before AS pending_before,
    v_after AS pending_after,
    v_active_rules AS active_rules;
END$$

DROP PROCEDURE IF EXISTS `sp_aml_rule_centif_daily_15m`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_aml_rule_centif_daily_15m` (IN `p_transaction_id` BIGINT)   main_centif:
BEGIN
  DECLARE v_client_id BIGINT DEFAULT NULL;
  DECLARE v_account_id BIGINT DEFAULT NULL;
  DECLARE v_amount DECIMAL(18,2) DEFAULT 0;
  DECLARE v_tx_type VARCHAR(50) DEFAULT NULL;
  DECLARE v_tx_date DATETIME DEFAULT NULL;
  DECLARE v_tx_status VARCHAR(50) DEFAULT NULL;

  DECLARE v_rule_id BIGINT DEFAULT NULL;
  DECLARE v_rule_score DECIMAL(5,2) DEFAULT 70;
  DECLARE v_threshold DECIMAL(18,2) DEFAULT 15000000;
  DECLARE v_include_types VARCHAR(255) DEFAULT 'DEPOSIT,WITHDRAWAL';
  DECLARE v_day_start DATETIME DEFAULT NULL;
  DECLARE v_day_end DATETIME DEFAULT NULL;
  DECLARE v_daily_volume DECIMAL(18,2) DEFAULT 0;
  DECLARE v_match TINYINT DEFAULT 0;
  DECLARE v_type_ok TINYINT DEFAULT 0;
  ...
  IF v_client_id IS NULL OR v_tx_date IS NULL THEN
    LEAVE main_centif;
  END IF;

  IF UPPER(COALESCE(v_tx_status, 'COMPLETED')) IN ('CANCELLED', 'CANCELED', 'REVERSED', 'VOID') THEN
    LEAVE main_centif;
  END IF;

  /* Règle active ? */
  SELECT id, COALESCE(score, 70)
  INTO v_rule_id, v_rule_score
  FROM aml_rules
  WHERE rule_code = 'CENTIF_DAILY_15M' AND active = 1
  LIMIT 1;
  ...
  /* Paramètres */
  SELECT
    COALESCE(MAX(CASE WHEN parameter_code = 'daily_threshold' THEN CAST(parameter_value AS DECIMAL(18,2)) END), 15000000),
    COALESCE(MAX(CASE WHEN parameter_code = 'include_types' THEN parameter_value END), 'DEPOSIT,WITHDRAWAL')
  INTO v_threshold, v_include_types
  FROM aml_rule_parameters
  WHERE rule_id = v_rule_id AND is_active = 1;

  /* Type courant inclus ? (liste CSV simple) */
  SET v_type_ok = IF(
    FIND_IN_SET(UPPER(TRIM(v_tx_type)), REPLACE(UPPER(REPLACE(v_include_types, ' ', '')), ',', ',')) > 0,
    1,
    0
  );

  /* Si le type courant n'est pas dans la liste, on évalue quand même le cumul
     du jour pour les types inclus (la TX peut être le déclencheur d'observation).
     Match seulement si cumul jour >= seuil. */

  SET v_day_start = DATE(v_tx_date);
  SET v_day_end   = DATE_ADD(v_day_start, INTERVAL 1 DAY);
  ...
  INNER JOIN accounts a ON a.id = t.account_id
  WHERE a.client_id = v_client_id
    AND t.transaction_date >= v_day_start
    AND t.transaction_date < v_day_end
    AND UPPER(COALESCE(t.transaction_status, 'COMPLETED')) NOT IN ('CANCELLED', 'CANCELED', 'REVERSED', 'VOID')
    AND FIND_IN_SET(
          UPPER(TRIM(t.transaction_type)),
          REPLACE(UPPER(REPLACE(v_include_types, ' ', '')), ',', ',')
        ) > 0;

  SET v_match = IF(v_daily_volume >= v_threshold, 1, 0);

  /* rule_executions */
  INSERT INTO rule_executions (rule_id, transaction_id, execution_result)
  VALUES (v_rule_id, p_transaction_id, IF(v_match = 1, 'MATCH', 'NO_MATCH'))
  ON DUPLICATE KEY UPDATE
    execution_result = VALUES(execution_result),
    executed_at = CURRENT_TIMESTAMP;
  ...
  IF v_match = 1 THEN
    INSERT INTO risk_assessments (
      client_id, transaction_id, risk_type, score, risk_level, reason, source
    ) VALUES (
      v_client_id,
      p_transaction_id,
      'CENTIF_DAILY_15M',
      LEAST(100, v_rule_score),
      CASE
      ...
      END,
      CONCAT(
        'Cumul journalier client (types: ', v_include_types, ') = ',
        FORMAT(v_daily_volume, 2),
        ' XOF >= seuil CENTIF ',
        FORMAT(v_threshold, 2),
        ' XOF (journée ', DATE_FORMAT(v_day_start, '%Y-%m-%d'), ').'
      ),
      'AML_RULE_ENGINE'
    )
    ON DUPLICATE KEY UPDATE
    ...
    /* Remonter le risque client si nécessaire (sans baisser) */
    UPDATE clients c
    SET
      risk_score = GREATEST(
        COALESCE(c.risk_score, 0),
        LEAST(100, v_rule_score)
      ),
      risk_level_id = (
        SELECT rl.id FROM risk_levels rl
        WHERE rl.code = CASE
          WHEN GREATEST(COALESCE(c.risk_score, 0), LEAST(100, v_rule_score)) >= 80 THEN 'CRITICAL'
          WHEN GREATEST(COALESCE(c.risk_score, 0), LEAST(100, v_rule_score)) >= 60 THEN 'HIGH'
          WHEN GREATEST(COALESCE(c.risk_score, 0), LEAST(100, v_rule_score)) >= 30 THEN 'MEDIUM'
          ELSE 'LOW'
        END
        LIMIT 1
      )
  ...
  ELSE
    DELETE FROM risk_assessments
    WHERE transaction_id = p_transaction_id
      AND risk_type = 'CENTIF_DAILY_15M'
      AND source = 'AML_RULE_ENGINE';
  END IF;

END$$

DROP PROCEDURE IF EXISTS `sp_client_onboard_individual`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_client_onboard_individual` (IN `p_agency_id` BIGINT, IN `p_client_number` VARCHAR(50), IN `p_phone` VARCHAR(30), IN `p_email` VARCHAR(150), IN `p_first_name` VARCHAR(100), IN `p_last_name` VARCHAR(100), IN `p_gender` VARCHAR(20), IN `p_birth_date` DATE, IN `p_place_of_birth` VARCHAR(150), IN `p_nationality` VARCHAR(50), IN `p_nina` VARCHAR(50), IN `p_marital_status` VARCHAR(30), IN `p_profession` VARCHAR(150), IN `p_activity_sector` VARCHAR(150), IN `p_declared_income` DECIMAL(18,2), IN `p_income_source` VARCHAR(100), IN `p_employer_name` VARCHAR(200), IN `p_photo_path` VARCHAR(500), IN `p_address_country` VARCHAR(100), IN `p_address_city` VARCHAR(100), IN `p_address_text` TEXT, IN `p_doc_type` VARCHAR(50), IN `p_doc_number` VARCHAR(100), IN `p_doc_issue_date` DATE, IN `p_doc_expiry_date` DATE, IN `p_doc_path` TEXT, IN `p_is_pep` TINYINT, IN `p_is_rca` TINYINT, IN `p_rca_note` VARCHAR(255), IN `p_run_screening` TINYINT, OUT `p_client_id` BIGINT, OUT `p_status` VARCHAR(30), OUT `p_message` VARCHAR(500))   main_onboard:
BEGIN
  DECLARE v_risk_low BIGINT DEFAULT NULL;
  DECLARE v_kyc_level VARCHAR(30) DEFAULT 'PENDING';
  DECLARE v_doc_id BIGINT DEFAULT NULL;
  ...
  BEGIN
    ...
    SET p_status = 'ERROR';
    SET p_message = 'Echec onboarding client (SQL exception).';
  END;
  ...
  IF p_agency_id IS NULL OR p_client_number IS NULL OR TRIM(p_client_number) = '' THEN
    SET p_message = 'agency_id et client_number obligatoires.';
    LEAVE main_onboard;
  END IF;

  IF p_first_name IS NULL OR p_last_name IS NULL THEN
    SET p_message = 'first_name et last_name obligatoires.';
    LEAVE main_onboard;
  END IF;

  IF EXISTS (SELECT 1 FROM clients WHERE client_number = p_client_number) THEN
    SET p_message = CONCAT('client_number déjà utilisé: ', p_client_number);
    LEAVE main_onboard;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM agencies WHERE id = p_agency_id) THEN
    SET p_message = 'agency_id introuvable.';
    LEAVE main_onboard;
  END IF;

  SET v_risk_low = (SELECT id FROM risk_levels WHERE code = 'LOW' LIMIT 1);

  /* Score KYC minimal (aligné vue v_client_kyc_completeness) */
  SET v_kyc_level = 'INCOMPLETE';
  IF p_phone IS NOT NULL AND p_nina IS NOT NULL AND p_birth_date IS NOT NULL
     AND p_profession IS NOT NULL AND p_doc_number IS NOT NULL THEN
    SET v_kyc_level = 'PARTIAL';
  END IF;
  IF p_phone IS NOT NULL AND p_nina IS NOT NULL AND p_birth_date IS NOT NULL
     AND p_profession IS NOT NULL AND p_declared_income IS NOT NULL
     AND p_income_source IS NOT NULL AND p_doc_number IS NOT NULL
     AND p_address_city IS NOT NULL THEN
    SET v_kyc_level = 'COMPLETE';
  END IF;
  ...
  INSERT INTO clients (
    ...
    is_pep, is_rca, rca_note,
    risk_level_id, risk_score, agency_id,
    kyc_status, kyc_completed_at,
    pep_approval_status
  ) VALUES (
    p_client_number, 'INDIVIDUAL', 'ACTIVE', p_phone, p_email,
    COALESCE(p_is_pep, 0), COALESCE(p_is_rca, 0), p_rca_note,
    v_risk_low, 0, p_agency_id,
    v_kyc_level,
    IF(v_kyc_level = 'COMPLETE', NOW(), NULL),
    IF(COALESCE(p_is_pep, 0) = 1, 'PENDING', 'NOT_REQUIRED')
  );

  SET p_client_id = LAST_INSERT_ID();

  INSERT INTO client_individuals (
    client_id, first_name, last_name, gender, marital_status,
    birth_date, place_of_birth, nationality, nina,
    profession, activity_sector, declared_income, income_source,
    employer_name, photo_path
  ) VALUES (
    p_client_id, p_first_name, p_last_name, p_gender, p_marital_status,
    p_birth_date, p_place_of_birth, COALESCE(p_nationality, 'ML'), p_nina,
    p_profession, p_activity_sector, p_declared_income, p_income_source,
    p_employer_name, p_photo_path
  );

  IF p_address_city IS NOT NULL OR p_address_text IS NOT NULL THEN
    INSERT INTO addresses (client_id, country, city, address)
    VALUES (p_client_id, COALESCE(p_address_country, 'ML'), p_address_city, p_address_text);
  END IF;

  IF p_doc_type IS NOT NULL OR p_doc_number IS NOT NULL THEN
    INSERT INTO identity_documents (
      client_id, document_type, document_number, issuing_country,
      issue_date, expiry_date, is_primary, document_path
    ) VALUES (
      p_client_id, p_doc_type, p_doc_number, 'ML',
      p_doc_issue_date, p_doc_expiry_date, 1, p_doc_path
    );
    SET v_doc_id = LAST_INSERT_ID();

    INSERT INTO kyc_document_checks (
      document_id, verification_method, verification_result, confidence_score
    ) VALUES (
      v_doc_id, 'MANUAL_AGENCY', 'PENDING', NULL
    );
  END IF;

  INSERT INTO kyc_reviews (client_id, review_date, review_status, review_comment)
  VALUES (
    p_client_id, CURDATE(),
    IF(v_kyc_level = 'COMPLETE', 'APPROVED_PRELIM', 'PENDING'),
    CONCAT('Onboarding procédure sp_client_onboard_individual — KYC ', v_kyc_level)
  );

  COMMIT;

  /* Screening hors transaction (échec screening ≠ rollback client) */
  IF COALESCE(p_run_screening, 1) = 1 THEN
    BEGIN
      DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN END;
      CALL sp_screen_client(p_client_id);
    END;
  END IF;

  SET p_status = 'SUCCESS';
  SET p_message = CONCAT('Client créé id=', p_client_id, ' kyc=', v_kyc_level);
END$$
...
DROP PROCEDURE IF EXISTS `sp_transaction_register`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_transaction_register` (IN `p_account_id` BIGINT, IN `p_transaction_reference` VARCHAR(100), IN `p_transaction_type` VARCHAR(30), IN `p_amount` DECIMAL(18,2), IN `p_currency` VARCHAR(10), IN `p_channel` VARCHAR(50), IN `p_country` VARCHAR(100), IN `p_transaction_date` DATETIME, IN `p_agency_id` BIGINT, IN `p_initiated_by_type` VARCHAR(30), IN `p_initiated_by_mandate_id` BIGINT, IN `p_initiated_by_client_id` BIGINT, IN `p_counterpart_account_id` BIGINT, IN `p_counterpart_client_id` BIGINT, IN `p_counterpart_name` VARCHAR(200), OUT `p_transaction_id` BIGINT, OUT `p_status` VARCHAR(30), OUT `p_message` VARCHAR(500))   main_tx:
BEGIN
  DECLARE v_acc_status VARCHAR(30);
  DECLARE v_agency BIGINT;
  ...
  BEGIN
    ...
    SET p_status = 'ERROR';
    SET p_message = 'Echec enregistrement transaction.';
  END;
  ...
  IF p_account_id IS NULL OR p_amount IS NULL OR p_amount <= 0 THEN
    SET p_message = 'account_id et amount > 0 obligatoires.';
    LEAVE main_tx;
  END IF;

  IF p_transaction_type NOT IN ('DEPOSIT','WITHDRAWAL','TRANSFER_IN','TRANSFER_OUT','PAYMENT') THEN
    SET p_message = 'transaction_type invalide.';
    LEAVE main_tx;
  END IF;

  SELECT status INTO v_acc_status FROM accounts WHERE id = p_account_id LIMIT 1;
  IF v_acc_status IS NULL THEN
    SET p_message = 'Compte introuvable.';
    LEAVE main_tx;
  END IF;
  IF v_acc_status <> 'ACTIVE' THEN
    SET p_message = CONCAT('Compte non actif (', v_acc_status, ').');
    LEAVE main_tx;
  END IF;

  SET v_agency = p_agency_id;
  ...
  INSERT INTO transactions (
    ...
    COALESCE(p_transaction_reference, CONCAT('TX-', UNIX_TIMESTAMP(), '-', p_account_id)),
    p_account_id,
    p_transaction_type,
    p_amount,
    COALESCE(p_currency, 'XOF'),
    p_channel,
    COALESCE(p_country, 'ML'),
    COALESCE(p_transaction_date, NOW()),
    'COMPLETED',
    v_agency,
    p_initiated_by_type,
    p_initiated_by_mandate_id,
    p_initiated_by_client_id,
    p_counterpart_account_id,
    p_counterpart_client_id,
    p_counterpart_name
  );
  ...
  /* Si trigger désactivé, évaluation manuelle */
  BEGIN
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN END;
    CALL sp_aml_evaluate_transaction_silent(p_transaction_id);
  END;

  SET p_status = 'SUCCESS';
  SET p_message = CONCAT('Transaction enregistrée id=', p_transaction_id, ' — AML évalué.');
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_update_client_risk` (IN `p_client` BIGINT)   BEGIN
  ...
  SELECT LEAST(100, COALESCE(MAX(LEAST(100, ra.score)), 0))
  INTO v_score
  FROM risk_assessments ra
  WHERE ra.client_id = p_client;
...
CREATE TABLE IF NOT EXISTS `accounts` (
  ...
  `account_manager_id` bigint DEFAULT NULL COMMENT 'Gestionnaire de compte (users.id — role AGENT/SUPERVISOR)',
  PRIMARY KEY (`id`),
  UNIQUE KEY `account_number` (`account_number`),
  KEY `fk_accounts_client` (`client_id`),
  KEY `idx_accounts_currency` (`currency`),
  KEY `idx_accounts_manager` (`account_manager_id`)
...
CREATE TABLE IF NOT EXISTS `account_mandates` (
  ...
  `mandate_client_id` bigint DEFAULT NULL COMMENT 'Si le mandataire est déjà client CIF',
  `full_name` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Identité si tiers non client',
  `identity_number` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `phone` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `mandate_role` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'MANDATAIRE' COMMENT 'MANDATAIRE | TUTEUR | REPRESENTANT',
  `powers` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'WITHDRAWAL | DEPOSIT | ALL | ...',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_mandate_account_status` (`account_id`,`status`),
  KEY `idx_mandate_client` (`mandate_client_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `clients` (
  ...
  `is_rca` tinyint(1) NOT NULL DEFAULT '0' COMMENT '1 = proche / associé d''un PEP (RCA), PAS un PEP automatique',
  `rca_note` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Précision courte lien RCA (optionnel)',
  `kyc_status` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT 'PENDING' COMMENT 'PENDING | COMPLETE | INCOMPLETE | REVIEW_REQUIRED',
  `kyc_completed_at` datetime DEFAULT NULL,
  `pep_reviewed_at` datetime DEFAULT NULL COMMENT 'Dernière revue EDD si PEP',
  `pep_approval_status` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'NOT_REQUIRED | PENDING | APPROVED | REJECTED',
...
CREATE TABLE IF NOT EXISTS `client_individuals` (
  ...
  `last_name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `gender` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `marital_status` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'SINGLE | MARRIED | DIVORCED | WIDOWED | OTHER',
  `birth_date` date DEFAULT NULL,
  `place_of_birth` varchar(150) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `nationality` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `nina` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Numéro d''Identification Nationale (Mali)',
  `profession` varchar(150) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `activity_sector` varchar(150) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `declared_income` decimal(18,2) DEFAULT NULL COMMENT 'Revenu mensuel déclaré (XOF)',
  `income_source` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'SALARY | BUSINESS | AGRICULTURE | REMITTANCE | OTHER',
  `employer_name` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `photo_path` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Lien fichier photo identité (serveur / stockage)',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`client_id`),
  ...
  KEY `idx_individual_nina` (`nina`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- --------------------------------------------------------

--
-- Table structure for table `client_related_parties`
--

DROP TABLE IF EXISTS `client_related_parties`;
CREATE TABLE IF NOT EXISTS `client_related_parties` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `client_id` bigint NOT NULL COMMENT 'Client CIF objet du lien',
  `related_client_id` bigint DEFAULT NULL COMMENT 'Si le proche est aussi client de la CIF',
  `related_full_name` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Identité si tiers non client',
  `related_nina` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `relation_type` varchar(40) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'OTHER' COMMENT 'SPOUSE | CHILD | PARENT | SIBLING | ASSOCIATE | UBO | SHAREHOLDER | OTHER',
  `is_pep_link` tinyint(1) NOT NULL DEFAULT '0' COMMENT '1 si la partie liée est (ou était) PEP',
  `risk_relevance` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'MEDIUM' COMMENT 'LOW | MEDIUM | HIGH — appréciation conformité',
  `status` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'ACTIVE' COMMENT 'ACTIVE | ENDED',
  `notes` varchar(500) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_crp_client` (`client_id`,`status`),
  KEY `idx_crp_related_client` (`related_client_id`),
  KEY `idx_crp_pep_link` (`is_pep_link`,`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `identity_documents` (
  ...
  `issuing_country` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT 'ML',
  `issue_date` date DEFAULT NULL,
  `expiry_date` date DEFAULT NULL,
  `is_primary` tinyint(1) NOT NULL DEFAULT '0' COMMENT '1 = pièce principale pour KYC',
  `document_path` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,
  PRIMARY KEY (`id`),
  KEY `fk_identity_documents_client` (`client_id`),
  KEY `idx_id_doc_number` (`document_number`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `risk_assessments` (
  ...
  KEY `idx_ra_client_type_created` (`client_id`,`risk_type`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `transactions` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `agency_id` bigint NOT NULL,
  `counterpart_account_id` bigint DEFAULT NULL COMMENT 'Compte CIF contrepartie (transfert interne)',
  `counterpart_client_id` bigint DEFAULT NULL COMMENT 'Client contrepartie (dénormalisé)',
  `counterpart_name` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Libellé bénéficiaire / donneur si externe',
  `counterpart_external_ref` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Réf mobile money / banque externe',
  `initiated_by_type` varchar(30) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'HOLDER | MANDATE | AGENT | SYSTEM',
  `initiated_by_mandate_id` bigint DEFAULT NULL COMMENT 'FK account_mandates si MANDATE',
  `initiated_by_client_id` bigint DEFAULT NULL COMMENT 'Client opérateur si connu',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_tx_initiated_by_type` (`initiated_by_type`),
  KEY `idx_tx_initiated_by_mandate` (`initiated_by_mandate_id`),
  KEY `idx_tx_initiated_by_client` (`initiated_by_client_id`),
  KEY `idx_tx_date_status_type` (`transaction_date`,`transaction_status`,`transaction_type`),
  KEY `idx_tx_counterpart_account` (`counterpart_account_id`),
  KEY `idx_tx_counterpart_client` (`counterpart_client_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
--
-- Stand-in structure for view `v_account_tx_averages_30d`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `v_account_tx_averages_30d`;
CREATE TABLE IF NOT EXISTS `v_account_tx_averages_30d` (
`account_id` bigint
,`account_number` varchar(50)
,`account_status` varchar(30)
,`avg_amount_30d` decimal(22,6)
,`avg_deposit_30d` decimal(22,6)
,`avg_transfer_in_30d` decimal(22,6)
,`avg_transfer_out_30d` decimal(22,6)
,`avg_withdrawal_30d` decimal(22,6)
,`client_id` bigint
,`current_balance` decimal(18,2)
,`deposit_count_30d` decimal(23,0)
,`last_tx_at_30d` datetime
,`transfer_in_count_30d` decimal(23,0)
,`transfer_out_count_30d` decimal(23,0)
,`tx_count_30d` bigint
,`volume_30d` decimal(40,2)
,`withdrawal_count_30d` decimal(23,0)
);
...
--
-- Stand-in structure for view `v_client_counterparty_links`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `v_client_counterparty_links`;
CREATE TABLE IF NOT EXISTS `v_client_counterparty_links` (
`first_tx_at` datetime
,`inflow_amount` decimal(40,2)
,`last_tx_at` datetime
,`outflow_amount` decimal(40,2)
...
--
-- Stand-in structure for view `v_client_dual_risk_profile`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `v_client_dual_risk_profile`;
CREATE TABLE IF NOT EXISTS `v_client_dual_risk_profile` (
`aml_assessment_count` bigint
,`aml_risk_level_code` varchar(30)
,`aml_risk_level_label` varchar(100)
,`aml_risk_score` decimal(5,2)
,`aml_rule_types_30d` text
,`centif_hit_count` bigint
,`client_id` bigint
,`client_number` varchar(50)
,`client_status` enum('ACTIVE','SUSPENDED','BLOCKED','CLOSED')
,`is_pep` int
...
,`ml_ai_score` decimal(5,2)
,`ml_final_score` decimal(5,2)
,`ml_latest_predicted_risk` varchar(30)
,`ml_latest_probability` decimal(5,2)
,`ml_pep_score` decimal(5,2)
,`ml_risk_decision` varchar(50)
,`ml_transaction_score` decimal(5,2)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `v_client_kyc_completeness`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `v_client_kyc_completeness`;
CREATE TABLE IF NOT EXISTS `v_client_kyc_completeness` (
...
,`kyc_completeness_level` varchar(10)
,`kyc_score_max` int
,`kyc_score_points` int
,`kyc_status` varchar(30)
,`last_name` varchar(100)
,`marital_status` varchar(30)
,`national_id_count` bigint
,`nationality` varchar(50)
,`nina` varchar(50)
,`pep_approval_status` varchar(30)
,`phone` varchar(30)
...
--
-- Stand-in structure for view `v_client_onboarding_status`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `v_client_onboarding_status`;
CREATE TABLE IF NOT EXISTS `v_client_onboarding_status` (
`active_accounts` bigint
,`client_id` bigint
,`client_number` varchar(50)
,`client_status` enum('ACTIVE','SUSPENDED','BLOCKED','CLOSED')
,`documents_count` bigint
,`documents_with_file_count` bigint
,`is_pep` int
,`is_rca` int
,`kyc_completed_at` datetime
,`kyc_status` varchar(30)
,`last_kyc_review_status` varchar(50)
,`onboarding_stage` varchar(20)
,`pep_approval_status` varchar(30)
);
...
--
-- Stand-in structure for view `v_client_pep_rca_profile`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `v_client_pep_rca_profile`;
CREATE TABLE IF NOT EXISTS `v_client_pep_rca_profile` (
`client_id` bigint
,`client_number` varchar(50)
,`client_status` enum('ACTIVE','SUSPENDED','BLOCKED','CLOSED')
,`client_type` varchar(50)
,`exposure_class` varchar(12)
,`is_pep` int
,`is_rca` int
,`pep_approval_status` varchar(30)
,`pep_links_count` bigint
,`pep_reviewed_at` datetime
,`rca_note` varchar(255)
,`related_parties_count` bigint
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `v_client_tx_averages_30d`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `v_client_tx_averages_30d`;
CREATE TABLE IF NOT EXISTS `v_client_tx_averages_30d` (
`avg_amount_30d` decimal(22,6)
,`avg_deposit_30d` decimal(22,6)
,`avg_transfer_in_30d` decimal(22,6)
,`avg_transfer_out_30d` decimal(22,6)
...
,`client_type` varchar(50)
,`deposit_count_30d` decimal(23,0)
,`deposit_volume_30d` decimal(40,2)
,`is_pep` int
,`is_rca` int
,`last_tx_at_30d` datetime
,`max_amount_30d` decimal(18,2)
,`outflow_inflow_ratio_30d` decimal(46,6)
,`payment_count_30d` decimal(23,0)
,`payment_volume_30d` decimal(40,2)
,`transfer_in_count_30d` decimal(23,0)
,`transfer_in_volume_30d` decimal(40,2)
,`transfer_out_count_30d` decimal(23,0)
,`transfer_out_volume_30d` decimal(40,2)
,`tx_count_30d` bigint
,`volume_30d` decimal(40,2)
,`withdrawal_count_30d` decimal(23,0)
,`withdrawal_volume_30d` decimal(40,2)
);
...
--
-- Structure for view `v_account_tx_averages_30d`
--
DROP TABLE IF EXISTS `v_account_tx_averages_30d`;

DROP VIEW IF EXISTS `v_account_tx_averages_30d`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_account_tx_averages_30d`  AS SELECT `a`.`id` AS `account_id`, `a`.`account_number` AS `account_number`, `a`.`client_id` AS `client_id`, `a`.`status` AS `account_status`, `a`.`current_balance` AS `current_balance`, count(`t`.`id`) AS `tx_count_30d`, coalesce(sum(`t`.`amount`),0) AS `volume_30d`, coalesce(avg(`t`.`amount`),0) AS `avg_amount_30d`, sum((case when (`t`.`transaction_type` = 'DEPOSIT') then 1 else 0 end)) AS `deposit_count_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'DEPOSIT') then `t`.`amount` end)),0) AS `avg_deposit_30d`, sum((case when (`t`.`transaction_type` = 'WITHDRAWAL') then 1 else 0 end)) AS `withdrawal_count_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'WITHDRAWAL') then `t`.`amount` end)),0) AS `avg_withdrawal_30d`, sum((case when (`t`.`transaction_type` = 'TRANSFER_OUT') then 1 else 0 end)) AS `transfer_out_count_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'TRANSFER_OUT') then `t`.`amount` end)),0) AS `avg_transfer_out_30d`, sum((case when (`t`.`transaction_type` = 'TRANSFER_IN') then 1 else 0 end)) AS `transfer_in_count_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'TRANSFER_IN') then `t`.`amount` end)),0) AS `avg_transfer_in_30d`, max(`t`.`transaction_date`) AS `last_tx_at_30d` FROM (`accounts` `a` left join `transactions` `t` on(((`t`.`account_id` = `a`.`id`) and (`t`.`transaction_date` >= (now() - interval 30 day)) and (upper(coalesce(`t`.`transaction_status`,'COMPLETED')) not in ('CANCELLED','CANCELED','REVERSED','VOID'))))) GROUP BY `a`.`id`, `a`.`account_number`, `a`.`client_id`, `a`.`status`, `a`.`current_balance` ;
...
--
-- Structure for view `v_client_counterparty_links`
--
DROP TABLE IF EXISTS `v_client_counterparty_links`;

DROP VIEW IF EXISTS `v_client_counterparty_links`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_client_counterparty_links`  AS SELECT `src`.`client_id` AS `source_client_id`, `t`.`counterpart_client_id` AS `target_client_id`, count(0) AS `tx_count`, sum(`t`.`amount`) AS `total_amount`, min(`t`.`transaction_date`) AS `first_tx_at`, max(`t`.`transaction_date`) AS `last_tx_at`, sum((case when (`t`.`transaction_type` in ('TRANSFER_OUT','PAYMENT','WITHDRAWAL')) then `t`.`amount` else 0 end)) AS `outflow_amount`, sum((case when (`t`.`transaction_type` in ('TRANSFER_IN','DEPOSIT')) then `t`.`amount` else 0 end)) AS `inflow_amount` FROM (`transactions` `t` join `accounts` `src` on((`src`.`id` = `t`.`account_id`))) WHERE ((`t`.`counterpart_client_id` is not null) AND (upper(coalesce(`t`.`transaction_status`,'COMPLETED')) not in ('CANCELLED','CANCELED','REVERSED','VOID')) AND (`t`.`transaction_date` >= (curdate() - interval 90 day))) GROUP BY `src`.`client_id`, `t`.`counterpart_client_id` HAVING ((count(0) >= 5) OR (sum(`t`.`amount`) >= 5000000)) ;

-- --------------------------------------------------------

--
-- Structure for view `v_client_dual_risk_profile`
--
DROP TABLE IF EXISTS `v_client_dual_risk_profile`;

DROP VIEW IF EXISTS `v_client_dual_risk_profile`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_client_dual_risk_profile`  AS SELECT `c`.`id` AS `client_id`, `c`.`client_number` AS `client_number`, `c`.`status` AS `client_status`, coalesce(`c`.`is_pep`,0) AS `is_pep`, coalesce(`c`.`is_rca`,0) AS `is_rca`, `c`.`kyc_status` AS `kyc_status`, coalesce(`c`.`risk_score`,0) AS `aml_risk_score`, `rl`.`code` AS `aml_risk_level_code`, `rl`.`label` AS `aml_risk_level_label`, (select count(0) from `risk_assessments` `ra` where ((`ra`.`client_id` = `c`.`id`) and (`ra`.`source` = 'AML_RULE_ENGINE'))) AS `aml_assessment_count`, (select count(0) from `risk_assessments` `ra` where ((`ra`.`client_id` = `c`.`id`) and (`ra`.`risk_type` = 'CENTIF_DAILY_15M'))) AS `centif_hit_count`, (select group_concat(distinct `ra`.`risk_type` order by `ra`.`risk_type` ASC separator ',') from `risk_assessments` `ra` where ((`ra`.`client_id` = `c`.`id`) and (`ra`.`source` = 'AML_RULE_ENGINE') and (`ra`.`created_at` >= (now() - interval 30 day)))) AS `aml_rule_types_30d`, (select `rs`.`ai_score` from `risk_scores` `rs` where (`rs`.`client_id` = `c`.`id`) order by `rs`.`created_at` desc,`rs`.`id` desc limit 1) AS `ml_ai_score`, (select `rs`.`pep_score` from `risk_scores` `rs` where (`rs`.`client_id` = `c`.`id`) order by `rs`.`created_at` desc,`rs`.`id` desc limit 1) AS `ml_pep_score`, (select `rs`.`transaction_score` from `risk_scores` `rs` where (`rs`.`client_id` = `c`.`id`) order by `rs`.`created_at` desc,`rs`.`id` desc limit 1) AS `ml_transaction_score`, (select `rs`.`final_score` from `risk_scores` `rs` where (`rs`.`client_id` = `c`.`id`) order by `rs`.`created_at` desc,`rs`.`id` desc limit 1) AS `ml_final_score`, (select `rs`.`risk_decision` from `risk_scores` `rs` where (`rs`.`client_id` = `c`.`id`) order by `rs`.`created_at` desc,`rs`.`id` desc limit 1) AS `ml_risk_decision`, (select `p`.`probability` from `predictions` `p` where (`p`.`client_id` = `c`.`id`) order by `p`.`predicted_at` desc,`p`.`id` desc limit 1) AS `ml_latest_probability`, (select `p`.`predicted_risk` from `predictions` `p` where (`p`.`client_id` = `c`.`id`) order by `p`.`predicted_at` desc,`p`.`id` desc limit 1) AS `ml_latest_predicted_risk` FROM (`clients` `c` left join `risk_levels` `rl` on((`rl`.`id` = `c`.`risk_level_id`))) ;

-- --------------------------------------------------------

--
-- Structure for view `v_client_kyc_completeness`
--
DROP TABLE IF EXISTS `v_client_kyc_completeness`;

DROP VIEW IF EXISTS `v_client_kyc_completeness`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_client_kyc_completeness`  AS SELECT `c`.`id` AS `client_id`, `c`.`client_number` AS `client_number`, `c`.`client_type` AS `client_type`, `c`.`status` AS `client_status`, `c`.`phone` AS `phone`, `c`.`email` AS `email`, `c`.`is_pep` AS `is_pep`, `c`.`kyc_status` AS `kyc_status`, `c`.`kyc_completed_at` AS `kyc_completed_at`, `c`.`pep_approval_status` AS `pep_approval_status`, `ci`.`first_name` AS `first_name`, `ci`.`last_name` AS `last_name`, `ci`.`nina` AS `nina`, `ci`.`birth_date` AS `birth_date`, `ci`.`place_of_birth` AS `place_of_birth`, `ci`.`nationality` AS `nationality`, `ci`.`marital_status` AS `marital_status`, `ci`.`profession` AS `profession`, `ci`.`activity_sector` AS `activity_sector`, `ci`.`declared_income` AS `declared_income`, `ci`.`income_source` AS `income_source`, `ci`.`employer_name` AS `employer_name`, (select count(0) from `identity_documents` `d` where (`d`.`client_id` = `c`.`id`)) AS `document_count`, (select count(0) from `identity_documents` `d` where ((`d`.`client_id` = `c`.`id`) and (`d`.`expiry_date` is not null) and (`d`.`expiry_date` < curdate()))) AS `expired_document_count`, (select count(0) from `identity_documents` `d` where ((`d`.`client_id` = `c`.`id`) and (upper(coalesce(`d`.`document_type`,'')) in ('NINA','CNI','NATIONAL_ID','CARTE_IDENTITE')))) AS `national_id_count`, (((((((((`c`.`phone` is not null) and (trim(`c`.`phone`) <> '')) + ((`ci`.`nina` is not null) and (trim(`ci`.`nina`) <> ''))) + (`ci`.`birth_date` is not null)) + ((`ci`.`profession` is not null) and (trim(`ci`.`profession`) <> ''))) + (`ci`.`declared_income` is not null)) + ((`ci`.`income_source` is not null) and (trim(`ci`.`income_source`) <> ''))) + exists(select 1 from `identity_documents` `d` where ((`d`.`client_id` = `c`.`id`) and ((`d`.`expiry_date` is null) or (`d`.`expiry_date` >= curdate()))))) + exists(select 1 from `addresses` `a` where (`a`.`client_id` = `c`.`id`))) AS `kyc_score_points`, 8 AS `kyc_score_max`, (case when (`c`.`client_type` = 'ENTITY') then NULL when ((((((((((`c`.`phone` is not null) and (trim(`c`.`phone`) <> '')) + ((`ci`.`nina` is not null) and (trim(`ci`.`nina`) <> ''))) + (`ci`.`birth_date` is not null)) + ((`ci`.`profession` is not null) and (trim(`ci`.`profession`) <> ''))) + (`ci`.`declared_income` is not null)) + ((`ci`.`income_source` is not null) and (trim(`ci`.`income_source`) <> ''))) + exists(select 1 from `identity_documents` `d` where ((`d`.`client_id` = `c`.`id`) and ((`d`.`expiry_date` is null) or (`d`.`expiry_date` >= curdate()))))) + exists(select 1 from `addresses` `a` where (`a`.`client_id` = `c`.`id`))) >= 7) then 'COMPLETE' when ((((((((((`c`.`phone` is not null) and (trim(`c`.`phone`) <> '')) + ((`ci`.`nina` is not null) and (trim(`ci`.`nina`) <> ''))) + (`ci`.`birth_date` is not null)) + ((`ci`.`profession` is not null) and (trim(`ci`.`profession`) <> ''))) + (`ci`.`declared_income` is not null)) + ((`ci`.`income_source` is not null) and (trim(`ci`.`income_source`) <> ''))) + exists(select 1 from `identity_documents` `d` where ((`d`.`client_id` = `c`.`id`) and ((`d`.`expiry_date` is null) or (`d`.`expiry_date` >= curdate()))))) + exists(select 1 from `addresses` `a` where (`a`.`client_id` = `c`.`id`))) >= 4) then 'PARTIAL' else 'INCOMPLETE' end) AS `kyc_completeness_level` FROM (`clients` `c` left join `client_individuals` `ci` on((`ci`.`client_id` = `c`.`id`))) ;

-- --------------------------------------------------------

--
-- Structure for view `v_client_onboarding_status`
--
DROP TABLE IF EXISTS `v_client_onboarding_status`;

DROP VIEW IF EXISTS `v_client_onboarding_status`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_client_onboarding_status`  AS SELECT `c`.`id` AS `client_id`, `c`.`client_number` AS `client_number`, `c`.`status` AS `client_status`, `c`.`kyc_status` AS `kyc_status`, `c`.`kyc_completed_at` AS `kyc_completed_at`, coalesce(`c`.`is_pep`,0) AS `is_pep`, coalesce(`c`.`is_rca`,0) AS `is_rca`, `c`.`pep_approval_status` AS `pep_approval_status`, (select count(0) from `identity_documents` `d` where (`d`.`client_id` = `c`.`id`)) AS `documents_count`, (select count(0) from `identity_documents` `d` where ((`d`.`client_id` = `c`.`id`) and (`d`.`document_path` is not null) and (trim(`d`.`document_path`) <> ''))) AS `documents_with_file_count`, (select count(0) from `accounts` `a` where ((`a`.`client_id` = `c`.`id`) and (`a`.`status` = 'ACTIVE'))) AS `active_accounts`, (select max(`kr`.`review_status`) from `kyc_reviews` `kr` where (`kr`.`client_id` = `c`.`id`)) AS `last_kyc_review_status`, (case when ((coalesce(`c`.`is_pep`,0) = 1) and (coalesce(`c`.`pep_approval_status`,'') <> 'APPROVED')) then 'PEP_PENDING_APPROVAL' when (`c`.`kyc_status` in ('INCOMPLETE','PENDING')) then 'KYC_INCOMPLETE' when ((select count(0) from `accounts` `a` where ((`a`.`client_id` = `c`.`id`) and (`a`.`status` = 'ACTIVE'))) = 0) then 'READY_FOR_ACCOUNT' else 'OPERATIONAL' end) AS `onboarding_stage` FROM `clients` AS `c` ;

-- --------------------------------------------------------

--
-- Structure for view `v_client_pep_rca_profile`
--
DROP TABLE IF EXISTS `v_client_pep_rca_profile`;

DROP VIEW IF EXISTS `v_client_pep_rca_profile`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_client_pep_rca_profile`  AS SELECT `c`.`id` AS `client_id`, `c`.`client_number` AS `client_number`, `c`.`client_type` AS `client_type`, `c`.`status` AS `client_status`, coalesce(`c`.`is_pep`,0) AS `is_pep`, coalesce(`c`.`is_rca`,0) AS `is_rca`, `c`.`rca_note` AS `rca_note`, `c`.`pep_approval_status` AS `pep_approval_status`, `c`.`pep_reviewed_at` AS `pep_reviewed_at`, (select count(0) from `client_related_parties` `p` where ((`p`.`client_id` = `c`.`id`) and (`p`.`status` = 'ACTIVE'))) AS `related_parties_count`, (select count(0) from `client_related_parties` `p` where ((`p`.`client_id` = `c`.`id`) and (`p`.`status` = 'ACTIVE') and (`p`.`is_pep_link` = 1))) AS `pep_links_count`, (case when (coalesce(`c`.`is_pep`,0) = 1) then 'PEP' when (coalesce(`c`.`is_rca`,0) = 1) then 'RCA' when exists(select 1 from `client_related_parties` `p` where ((`p`.`client_id` = `c`.`id`) and (`p`.`status` = 'ACTIVE') and (`p`.`is_pep_link` = 1))) then 'HAS_PEP_LINK' else 'STANDARD' end) AS `exposure_class` FROM `clients` AS `c` ;

-- --------------------------------------------------------

--
-- Structure for view `v_client_tx_averages_30d`
--
DROP TABLE IF EXISTS `v_client_tx_averages_30d`;

DROP VIEW IF EXISTS `v_client_tx_averages_30d`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_client_tx_averages_30d`  AS SELECT `c`.`id` AS `client_id`, `c`.`client_number` AS `client_number`, `c`.`client_type` AS `client_type`, `c`.`status` AS `client_status`, coalesce(`c`.`is_pep`,0) AS `is_pep`, coalesce(`c`.`is_rca`,0) AS `is_rca`, count(`t`.`id`) AS `tx_count_30d`, coalesce(sum(`t`.`amount`),0) AS `volume_30d`, coalesce(avg(`t`.`amount`),0) AS `avg_amount_30d`, coalesce(max(`t`.`amount`),0) AS `max_amount_30d`, sum((case when (`t`.`transaction_type` = 'DEPOSIT') then 1 else 0 end)) AS `deposit_count_30d`, coalesce(sum((case when (`t`.`transaction_type` = 'DEPOSIT') then `t`.`amount` else 0 end)),0) AS `deposit_volume_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'DEPOSIT') then `t`.`amount` end)),0) AS `avg_deposit_30d`, sum((case when (`t`.`transaction_type` = 'WITHDRAWAL') then 1 else 0 end)) AS `withdrawal_count_30d`, coalesce(sum((case when (`t`.`transaction_type` = 'WITHDRAWAL') then `t`.`amount` else 0 end)),0) AS `withdrawal_volume_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'WITHDRAWAL') then `t`.`amount` end)),0) AS `avg_withdrawal_30d`, sum((case when (`t`.`transaction_type` = 'TRANSFER_OUT') then 1 else 0 end)) AS `transfer_out_count_30d`, coalesce(sum((case when (`t`.`transaction_type` = 'TRANSFER_OUT') then `t`.`amount` else 0 end)),0) AS `transfer_out_volume_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'TRANSFER_OUT') then `t`.`amount` end)),0) AS `avg_transfer_out_30d`, sum((case when (`t`.`transaction_type` = 'TRANSFER_IN') then 1 else 0 end)) AS `transfer_in_count_30d`, coalesce(sum((case when (`t`.`transaction_type` = 'TRANSFER_IN') then `t`.`amount` else 0 end)),0) AS `transfer_in_volume_30d`, coalesce(avg((case when (`t`.`transaction_type` = 'TRANSFER_IN') then `t`.`amount` end)),0) AS `avg_transfer_in_30d`, sum((case when (`t`.`transaction_type` = 'PAYMENT') then 1 else 0 end)) AS `payment_count_30d`, coalesce(sum((case when (`t`.`transaction_type` = 'PAYMENT') then `t`.`amount` else 0 end)),0) AS `payment_volume_30d`, (case when (coalesce(sum((case when (`t`.`transaction_type` in ('DEPOSIT','TRANSFER_IN')) then `t`.`amount` else 0 end)),0) = 0) then NULL else (coalesce(sum((case when (`t`.`transaction_type` in ('WITHDRAWAL','TRANSFER_OUT','PAYMENT')) then `t`.`amount` else 0 end)),0) / sum((case when (`t`.`transaction_type` in ('DEPOSIT','TRANSFER_IN')) then `t`.`amount` else 0 end))) end) AS `outflow_inflow_ratio_30d`, max(`t`.`transaction_date`) AS `last_tx_at_30d` FROM ((`clients` `c` left join `accounts` `a` on((`a`.`client_id` = `c`.`id`))) left join `transactions` `t` on(((`t`.`account_id` = `a`.`id`) and (`t`.`transaction_date` >= (now() - interval 30 day)) and (upper(coalesce(`t`.`transaction_status`,'COMPLETED')) not in ('CANCELLED','CANCELED','REVERSED','VOID'))))) GROUP BY `c`.`id`, `c`.`client_number`, `c`.`client_type`, `c`.`status`, `c`.`is_pep`, `c`.`is_rca` ;
...
ALTER TABLE `accounts`
  ...
  ADD CONSTRAINT `fk_accounts_manager` FOREIGN KEY (`account_manager_id`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;

--
-- Constraints for table `account_mandates`
--
ALTER TABLE `account_mandates`
  ADD CONSTRAINT `fk_mandate_account` FOREIGN KEY (`account_id`) REFERENCES `accounts` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_mandate_client` FOREIGN KEY (`mandate_client_id`) REFERENCES `clients` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
--
-- Constraints for table `client_related_parties`
--
ALTER TABLE `client_related_parties`
  ADD CONSTRAINT `fk_crp_client` FOREIGN KEY (`client_id`) REFERENCES `clients` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_crp_related_client` FOREIGN KEY (`related_client_id`) REFERENCES `clients` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
ALTER TABLE `transactions`
  ...
  ADD CONSTRAINT `fk_tx_counterpart_account` FOREIGN KEY (`counterpart_account_id`) REFERENCES `accounts` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_tx_counterpart_client` FOREIGN KEY (`counterpart_client_id`) REFERENCES `clients` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_tx_initiated_mandate` FOREIGN KEY (`initiated_by_mandate_id`) REFERENCES `account_mandates` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
